$(document).ready(function(){
	...
	$('body').click(function(e){
		...
		if(target.is('.client')){
			if(expanded==false){
				expandDetail(target.find('h2').text(), target.attr('name'), $('#client-list').height()-2*parseInt($('.cap-tech').css('margin-top')));
				expanded=true;
			}
		}
			...
			if(expanded==false){
				expandDetail(target.parents('.client').find('h2').text(), target.parents('.client').attr('name'), $('#client-list').height()-2*parseInt($('.cap-tech').css('margin-top')));
				expanded=true;
			}
		}
